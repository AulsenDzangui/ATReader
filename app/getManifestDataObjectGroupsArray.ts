...
export const getManifestDataObjectGroupsArray = (dataObjectPackageElement: string | any[] | HTMLCollectionOf<Element>) => {
    ...
    for (let i = 0; i < dataObjectPackageElement.length; i++) {
        ...
        for (let k = 0; k < binaryDataObjectsElement.length; k++) {
            ...
            const dataObjectGroupId = binaryDataObject.getElementsByTagName('DataObjectGroupId')[0]?.textContent;
            const dataObjectGroupIdAttribute = binaryDataObject.parentNode?.getAttribute('id');
