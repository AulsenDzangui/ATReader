...
function updateFileList(files) {
    ...
    if (files) {
        if (fileList && invitationDiv) {
            ...
            const sizeInKoItem1 = Math.round(((_a = files[1]) === null || _a === void 0 ? void 0 : _a.size) / 1000);
            ...
            <li>
                <span>${(_b = files[1]) === null || _b === void 0 ? void 0 : _b.name}</span> 
...
fileDropArea === null || fileDropArea === void 0 ? void 0 : fileDropArea.addEventListener('click', () => fileInput === null || fileInput === void 0 ? void 0 : fileInput.click());
fileInput === null || fileInput === void 0 ? void 0 : fileInput.addEventListener('change', (event) => {
    var _a;
...
items.forEach((item) => {
    const copyButton = item.lastElementChild; // Récupère le dernier enfant (le bouton de copie)
...
function renderObject(DataObjectGroupReferenceId, ArchiveUnitId) {
    try {
        ...
        fetch(url)
            ...
            if (!response.ok) {
                const errorData = yield response.text(); // Lire la réponse comme du texte
...
function renderObjectView(data) {
    ...
    if (parentElement) {
        ...
        <ul class="list-disc pl-5 space-y-3">
            ${atr.object.LogBook.map((log, index) => `
        ...
        function getParagraph(key, value) {
            const safeValue = value !== undefined ? value : '<i>Donnée absente</i>';
            return (`<p class="overflow-auto custom-scrollbar">
                    <strong>${key} : </strong>
                    <span>${safeValue}</span>
                </p>`);
        }
    ...
    if (button && button.parentElement && button.parentElement.parentElement) {
        {
            ...
            (_a = document.getElementById('modals')) === null || _a === void 0 ? void 0 : _a.classList.remove('flex');
        }
    }
}
